...
- For this exercise, you will receive (input) the amount of initial investment value.
- For each year, your forecaster colleague will give you (input) the predicted return.
- You will calculate the investment value for each year, over a period of 3 years.
...
- Your task is to calculate the investment value for each year, considering the influence of political parties, and determine, for each year, if the investment will increased or decreased compared to the previous year.
...
- Redo exercise [Exercise 1](#exercise-1) with lists and loops.
- Now, the user chooses the initial investment value and the number of years to simulate (use `input`).
- Your forecaster colleague will give you the prediction for the returns for those years (comma seperated `float` values).
...
- Show a formated table with the results, first column is the year, second column is the value in that year.
...
## Boston Housing Dataset - Data Normalization
...
# [Other exercises for edition 5](exercises_ed5.qmd)