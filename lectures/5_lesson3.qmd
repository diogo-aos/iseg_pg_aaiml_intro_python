...
- the function exits when it reaches the end of the instructions or when the instruction ``return`` is used
...
:::{.callout-note}
## Remember tuples?
When we return multiple values like this `a, b`, we're actually returning a `tuple`!

```python
def return_ab():
    return 1,2
type(return_ab()) # -> tuple
```
:::
...
We can have **local** and **global** variables
...
- `x` is a **global** variable, because it exists outside the scope of any function

```{.python code-line-numbers="[1,5,6,9,10]"} 
x = 3
...
```{.python code-line-numbers="[1,6,10]"} 
x = 3
...
```{.python code-line-numbers="[1,4,7,11]"}
x = 3
...
```text
      4     b = a + 2
----> 5     x += 2 # we try to change the value of the global variable x
      6     x = 4
      7     print(x)
      8     return b
...
Let's look at 2 particularly useful examples of using functions in this way.
...
Or, we can use `map`, which applies a function to all elements of an iterable (e.g. list).
...
In Python, we can create our own modules to store code and import that code into other programs.
...
Let's create a `dict` to store in a binary file.
...
#### Storing an object in a `pickle` file
...
<span style="color:gold">`Path`</span> is the main object to use, and it provides many methods to streamline file operations.
...
List all ``.py`` files inside  `code/`, recursively.
