...
- Organize data in data structures (lists, dictionaries)
...
::: {.callout-note}
## Actually...
Python has the `sum` function, no need to reinvent the wheel.
<!-- .element: class="fragment" -->
```python
total = sum(l)
```
...
To stop a loop, we can use the `break` instruction:
...
The types of both keys and values can be anything (that is hashable)
...
```python
for k in authors: # by default, the for iterates over keys
