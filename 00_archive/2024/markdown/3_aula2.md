...
- You are an analyst and should assign a return for each party (float, e.g. 5%).
- For each party, do a simulation for the evolution of an investment of 1000€ over 8 years.
- Print the party with the highest value at the end of the investment horizon, along with the full evolution for that party.
...
- Each party may make the return more or less volatile.
- In this exercise, we interpret the volatility as the standard deviation of the base return.
- As an analyst, you will assign a standard deviation for each party, based on the perceived uncertaint that party brings.
- Now, for each year, the yearly return to be applied to the investment is a random number with a normal distribution with the mean given by the base return and the assigned standard deviation to each party.
